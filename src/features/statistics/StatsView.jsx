...
export function StatsView({ transazioni, persone, meseOffset, categorie, goals, valutaBase = "EUR", activePersonaId = null, lang = "it" }) {
  ...
  const personalPerCategoria = activePerson
    ...
    : [];
  const ultimi6 = Array.from({length:6},(_,i)=>{const m=new Date(oggi.getFullYear(),oggi.getMonth()-(5-i),1);return{label:mese(m.getMonth()),valore:transazioni.filter(t=>t.tipo==="uscita"&&new Date(t.data).getMonth()===m.getMonth()&&new Date(t.data).getFullYear()===m.getFullYear()).reduce((s,t)=>s+t.importo,0),colore:color.accent};});
    ...
    <div>
      ...
      {activePerson && (personalEntrate > 0 || personalUscite > 0) && (
        <div style={{ background: color.surface, borderRadius: 20, padding: 20, marginBottom: 24, border: `1px solid ${activePerson.colore || color.accent}55` }}>
          ...
          <div style={{ marginBottom: personalPerCategoria.length > 0 ? 16 : 0 }}>
            <div style={{ display: "flex", gap: 10, marginBottom: 10 }}>
              <div style={{ flex: 1, minWidth: 0, background: color.bg, borderRadius: 14, padding: "12px 14px" }}>
                <div style={{ fontSize: 10, color: color.positive, letterSpacing: 0.5, textTransform: "uppercase" }}>{t(lang, "stats.income")}</div>
                <div style={{ fontSize: 15, fontWeight: 700, color: color.positive, fontFamily: moneyFont, fontVariantNumeric: "tabular-nums", marginTop: 4, whiteSpace: "nowrap" }}>{formattaValuta(personalEntrate)}</div>
              </div>
              <div style={{ flex: 1, minWidth: 0, background: color.bg, borderRadius: 14, padding: "12px 14px" }}>
                <div style={{ fontSize: 10, color: color.negative, letterSpacing: 0.5, textTransform: "uppercase" }}>{t(lang, "stats.expenses")}</div>
                <div style={{ fontSize: 15, fontWeight: 700, color: color.negative, fontFamily: moneyFont, fontVariantNumeric: "tabular-nums", marginTop: 4, whiteSpace: "nowrap" }}>{formattaValuta(personalUscite)}</div>
              </div>
            </div>
            <div style={{ background: color.bg, borderRadius: 14, padding: "12px 14px" }}>
              <div style={{ fontSize: 10, color: color.textSecondary, letterSpacing: 0.5, textTransform: "uppercase" }}>{t(lang, "stats.yourBalance")}</div>
              <div style={{ fontSize: 15, fontWeight: 700, color: personalSaldo >= 0 ? color.positive : color.negative, fontFamily: moneyFont, fontVariantNumeric: "tabular-nums", marginTop: 4, whiteSpace: "nowrap" }}>{personalSaldo >= 0 ? "+" : ""}{formattaValuta(personalSaldo)}</div>
            </div>
          </div>
          {personalPerCategoria.length > 0 && (
            <div>
              <div style={{ fontSize: 10, color: color.textMuted, letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 8 }}>{t(lang, "stats.yourExpensesByCategory")}</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {personalPerCategoria.slice(0, 5).map(c => (
                  <div key={c.id} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <span style={{ fontSize: 14 }}>{c.emoji}</span>
                    <span style={{ fontSize: 12, color: color.textSecondary, flex: 1 }}>{c.nome}</span>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 16 }}>
        <div style={{ flex: 1, background: color.surface, borderRadius: 16, padding: "16px 14px", border: `1px solid ${color.border}` }}>
          <div style={{ fontSize: 10, color: color.positive, letterSpacing: 0.5, textTransform: "uppercase" }}>{t(lang, "stats.income")}</div>
          <div style={{ fontSize: 18, fontWeight: 700, color: color.positive, fontFamily: moneyFont, fontVariantNumeric: "tabular-nums", marginTop: 4 }}>{formattaValuta(totalEntrate)}</div>
        </div>
        <div style={{ flex: 1, background: color.surface, borderRadius: 16, padding: "16px 14px", border: `1px solid ${color.border}` }}>
          <div style={{ fontSize: 10, color: color.negative, letterSpacing: 0.5, textTransform: "uppercase" }}>{t(lang, "stats.expenses")}</div>
          <div style={{ fontSize: 18, fontWeight: 700, color: color.negative, fontFamily: moneyFont, fontVariantNumeric: "tabular-nums", marginTop: 4 }}>{formattaValuta(totalUscite)}</div>
        </div>
      </div>
      {spesoPerPersona.some(p => p.speso > 0) && (
        <div style={{ background: color.surface, borderRadius: 16, padding: "14px 16px", marginBottom: 16, border: `1px solid ${color.border}` }}>
          <div style={{ fontSize: 11, color: color.textSecondary, letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 10 }}>{t(lang, "stats.whoPaid")}</div>
            ...
            {spesoPerPersona.map(p => (
              <div key={p.id} style={{ flex: "1 1 120px", display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 15, fontWeight: 700, color: color.textPrimary, fontFamily: moneyFont, fontVariantNumeric: "tabular-nums" }}>{formattaValuta(p.speso)}</div>
                </div>
              </div>
            ))}
          </div>
          {debitiMese.length > 0 && (
            <div style={{ borderTop: `1px solid ${color.border}`, paddingTop: 8, display: "flex", flexDirection: "column", gap: 4 }}>
              {debitiMese.map((d, i) => {
                const allP = getAllPersone(transazioni, persone);
                const pDa = allP.find(p => p.id === d.da) || { nome: d.da, emoji: "👤", colore: color.textMuted };
                const pA = allP.find(p => p.id === d.a) || { nome: d.a, emoji: "👤", colore: color.textMuted };
                return (
                  <div key={i} style={{ fontSize: 12, color: color.textSecondary }}>
                    <span>{pDa.emoji} {pDa.nome} {t(lang, "stats.owes")} <strong style={{ color: color.debt, fontFamily: moneyFont, fontVariantNumeric: "tabular-nums" }}>{formattaValuta(d.importo)}</strong> {t(lang, "stats.owesToConnector")} {pA.nome}</span>
                  </div>
                );
              })}
            </div>
        ...
        <div style={{ background: color.surface, borderRadius: 20, padding: 20, marginBottom: 24, border: `1px solid ${color.border}` }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", gap: 12 }}>
            {goals.map(g => {
              ...
              return (
                <div key={g.id} style={{ background: color.bg, borderRadius: 12, padding: 12, textAlign: "center" }}>
                  <GoalGauge current={g.currentAmount || 0} target={g.targetAmount} size={70} />
                  <div style={{ fontSize: 12, fontWeight: 600, color: color.textPrimary, marginTop: 8 }}>{g.nome}</div>
                  <div style={{ fontSize: 11, color: pct >= 100 ? color.positive : color.textMuted }}>{pct.toFixed(0)}%</div>
                  {daysLeft !== null && daysLeft < 30 && daysLeft >= 0 && (
                    <div style={{ fontSize: 10, color: color.warn, marginTop: 4 }}>{daysLeft}{t(lang, "stats.daysLeft")}</div>
                  )}
                </div>
              );
            })}
      ...
      <div style={{ background: color.surface, borderRadius: 20, padding: 20, border: `1px solid ${color.border}`, marginBottom: 24 }}>
        <div style={{ fontSize: 12, color: color.textSecondary, letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 16 }}>{t(lang, "stats.expenseTrend6m")}</div>
      ...
      {numTransazioni > 0 && (
        <>
          <div style={{ display: "flex", gap: 10, marginBottom: 16 }}>
            <div style={{ flex: 1, background: color.surface, borderRadius: 16, padding: "14px", border: `1px solid ${color.border}`, textAlign: "center" }}>
              <div style={{ fontSize: 10, color: color.textSecondary, letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 4 }}>{t(lang, "stats.transactionsCount")}</div>
              <div style={{ fontSize: 26, fontWeight: 800, color: color.accent, fontFamily: moneyFont, fontVariantNumeric: "tabular-nums" }}>{numTransazioni}</div>
            </div>
            <div style={{ flex: 1, background: color.surface, borderRadius: 16, padding: "14px", border: `1px solid ${color.border}`, textAlign: "center" }}>
              <div style={{ fontSize: 10, color: color.textSecondary, letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 4 }}>{t(lang, "stats.avgExpense")}</div>
              <div style={{ fontSize: 18, fontWeight: 800, color: color.negative, fontFamily: moneyFont, fontVariantNumeric: "tabular-nums" }}>{formattaValuta(spesaMedia)}</div>
            </div>
            <div style={{ flex: 1, background: color.surface, borderRadius: 16, padding: "14px", border: `1px solid ${color.border}`, textAlign: "center" }}>
              <div style={{ fontSize: 10, color: color.textSecondary, letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 4 }}>{t(lang, "stats.dailyFreq")}</div>
              <div style={{ fontSize: 18, fontWeight: 800, color: color.positive, fontFamily: moneyFont, fontVariantNumeric: "tabular-nums" }}>{(numTransazioni / giorniMese).toFixed(1)}<span style={{ fontSize: 11, color: color.textMuted }}>/g</span></div>
            </div>
          </div>
          <div style={{ background: color.surface, borderRadius: 20, padding: 20, marginBottom: 16, border: `1px solid ${color.border}` }}>
            <div style={{ fontSize: 12, color: color.textSecondary, letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 14 }}>{t(lang, "stats.amountDistribution")}</div>
          ...
          <div style={{ background: color.surface, borderRadius: 20, padding: 20, marginBottom: 24, border: `1px solid ${color.border}` }}>
            <div style={{ fontSize: 12, color: color.textSecondary, letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 14 }}>{t(lang, "stats.dailyHeatmap")}</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 4 }}>
              {perGiorno.map((g) => {
                const intensity = g.totale / maxGiorno;
                const bg = g.totale === 0 ? color.border : `rgba(124, 108, 240, ${0.15 + intensity * 0.85})`;
                return (
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 12, justifyContent: "center" }}>
              <span style={{ fontSize: 9, color: color.textMuted }}>{t(lang, "stats.less")}</span>
              {[0, 0.25, 0.5, 0.75, 1].map((v, i) => (
                <div key={i} style={{ width: 14, height: 14, borderRadius: 3, background: v === 0 ? color.border : `rgba(124, 108, 240, ${0.15 + v * 0.85})` }} />
              ))}
              <span style={{ fontSize: 9, color: color.textMuted }}>{t(lang, "stats.more")}</span>
            </div>
          </div>
        </>
      )}
      ...
      {(insights.length > 0 || catTrends.length > 0) && (
        <>
          ...
          {insights.length > 0 && (
            <div style={{ background: color.surface, borderRadius: 20, padding: 20, marginBottom: 16, border: `1px solid ${color.border}` }}>
              <div style={{ fontSize: 12, color: color.textSecondary, letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 14 }}>{t(lang, "stats.insightsTitle")}</div>
          ...
          {totalUscitePrec > 0 && (
            <div style={{ background: color.surface, borderRadius: 20, padding: 20, marginBottom: 16, border: `1px solid ${color.border}` }}>
              <div style={{ fontSize: 12, color: color.textSecondary, letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 14 }}>{t(lang, "stats.comparisonVs")} {mese(mesePrecedente.getMonth(), lang)}</div>
              {/* Uscite comparison */}
              <div style={{ marginBottom: 14 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                  <span style={{ fontSize: 11, color: color.textMuted }}>{t(lang, "stats.expenses")}</span>
                  <span style={{ fontSize: 12, fontWeight: 700, fontFamily: moneyFont, fontVariantNumeric: "tabular-nums", color: deltaPct > 0 ? color.negative : color.positive }}>
              ...
              {(totalEntrate > 0 || totalEntratePrec > 0) && (
                <div>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                    <span style={{ fontSize: 11, color: color.textMuted }}>{t(lang, "stats.income")}</span>
                    {deltaEntPct !== null && <span style={{ fontSize: 12, fontWeight: 700, fontFamily: moneyFont, fontVariantNumeric: "tabular-nums", color: deltaEntPct >= 0 ? color.positive : color.negative }}>
          ...
          {catTrends.length > 0 && totalUscitePrec > 0 && (
            <div style={{ background: color.surface, borderRadius: 20, padding: 20, marginBottom: 16, border: `1px solid ${color.border}` }}>
              <div style={{ fontSize: 12, color: color.textSecondary, letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 14 }}>{t(lang, "stats.categoryTrend")}</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {catTrends.slice(0, 6).map(c => (
                  <div key={c.id} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <span style={{ fontSize: 16, width: 24, textAlign: "center", flexShrink: 0 }}>{c.emoji}</span>
                    <span style={{ fontSize: 12, color: color.textSecondary, flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{c.nome}</span>
                    <span style={{ fontSize: 12, fontWeight: 600, fontFamily: moneyFont, fontVariantNumeric: "tabular-nums", color: color.textMuted, flexShrink: 0 }}>{formattaValuta(c.curr)}</span>
                    <span style={{
          ...
          {totalEntrate > 0 && (
            <div style={{ background: color.surface, borderRadius: 20, padding: 20, marginBottom: 24, border: `1px solid ${color.border}` }}>
              <div style={{ fontSize: 12, color: color.textSecondary, letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 14 }}>{t(lang, "stats.savingRate")}</div>
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                {/* Gauge ring */}
                <svg viewBox="0 0 80 80" width="70" height="70" style={{ flexShrink: 0 }}>
                  <circle cx="40" cy="40" r="32" fill="none" stroke={color.border} strokeWidth="8" />
                  <circle cx="40" cy="40" r="32" fill="none"
                    stroke={savingRate > 20 ? color.positive : savingRate > 0 ? color.warn : color.negative}
                    strokeWidth="8" strokeLinecap="round"
                    strokeDasharray={`${Math.max(0, Math.min(100, savingRate)) / 100 * 201} 201`}
                    transform="rotate(-90 40 40)" />
                  <text x="40" y="38" textAnchor="middle" fill={color.textPrimary} fontSize="14" fontWeight="800" fontFamily={moneyFont}>
                    {Math.round(savingRate)}%
                  </text>
                  <text x="40" y="50" textAnchor="middle" fill={color.textMuted} fontSize="7" fontFamily={displayFont}>{t(lang, "stats.savingsShort")}</text>
                </svg>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 12, color: color.textSecondary, marginBottom: 4 }}>
      ...
      <div style={{ background: color.surface, borderRadius: 20, padding: 20, marginBottom: 24, border: `1px solid ${color.border}` }}>
        ...
        {forecast.monthsUsed === 0 ? (
          ...
          <>
            <div style={{ fontSize: 28, fontWeight: 800, fontFamily: moneyFont, fontVariantNumeric: "tabular-nums", color: color.accent }}>
            ...
            {forecast.perCategory.length > 0 && (
              <div style={{ marginTop: 16 }}>
                <div style={{ fontSize: 10, color: color.textMuted, letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 8 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {forecast.perCategory.slice(0, 5).map(c => (
                    <div key={c.id} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <span style={{ fontSize: 14 }}>{c.emoji}</span>
                      <span style={{ fontSize: 12, color: color.textSecondary, flex: 1 }}>{c.nome}</span>
                      <span style={{ fontSize: 12, color: color.textSecondary, fontFamily: moneyFont, fontVariantNumeric: "tabular-nums" }}>{formattaValuta(c.valore)}</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {(() => {
        ...
        return (
          <div style={{ background: color.surface, borderRadius: 20, padding: "18px 16px", margin: "0 0 24px", border: `1px solid ${color.border}` }}>
            <div style={{ fontSize: 12, color: color.textSecondary, letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 14 }}>{t(lang, "stats.history12m")}</div>

            {/* Bar chart: entrate + uscite */}
            <div style={{ overflowX: "auto" }}>
              <svg viewBox={`0 0 ${CHART_W} ${CHART_H + 28}`} style={{ width: "100%", minWidth: 280 }}>
                {/* Zero line */}
                <line x1="0" y1={CHART_H} x2={CHART_W} y2={CHART_H} stroke={color.border} strokeWidth="1" />
                  ...
                  return (
                    <g key={m.ym}>
                      ...
                      <text x={x} y={CHART_H + 12} textAnchor="middle"
                        ...
                        fontFamily={displayFont} fontWeight={isCurrent ? "700" : "400"}>
                        {m.label}
                      </text>
                      <text x={x} y={CHART_H + 21} textAnchor="middle"
                        fill={color.textMuted} fontSize="6" fontFamily={displayFont}>
                ...
                {/* Saldo net line */}
                <polyline points={saldoPoints} fill="none" stroke={color.accent} strokeWidth="1.5"
                  strokeLinecap="round" strokeLinejoin="round" opacity="0.8" />
                {mesi12.map((m, i) => {
                  const x = gap * i + gap / 2;
                  const y = midY - (m.saldo / maxAbs) * (midY - 8);
                  return <circle key={m.ym} cx={x} cy={y} r="2.5" fill={color.accent} opacity="0.9" />;
                })}
              </svg>
            </div>
            ...
            <div style={{ display: "flex", gap: 16, marginTop: 4, justifyContent: "center" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 10, color: color.textMuted }}>
                <div style={{ width: 10, height: 10, borderRadius: 2, background: color.positive }} /> {t(lang, "stats.income")}
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 10, color: color.textMuted }}>
                <div style={{ width: 10, height: 10, borderRadius: 2, background: color.negative }} /> {t(lang, "stats.expenses")}
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 10, color: color.textMuted }}>
                <div style={{ width: 16, height: 2, borderRadius: 1, background: color.accent }} /> {t(lang, "stats.netBalance")}
              </div>
            </div>
            ...
            {(() => {
              ...
              return (
                <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
                  <div style={{ flex: 1, background: color.bg, borderRadius: 10, padding: "8px 10px" }}>
                    <div style={{ fontSize: 9, color: color.textMuted, textTransform: "uppercase", letterSpacing: 0.5 }}>{t(lang, "stats.bestMonth")}</div>
                    <div style={{ fontSize: 12, fontWeight: 700, color: color.positive, fontFamily: moneyFont, fontVariantNumeric: "tabular-nums", marginTop: 2 }}>{best.label}</div>
                    <div style={{ fontSize: 10, color: color.positive }}>{best.saldo >= 0 ? "+" : ""}{formattaValuta(best.saldo)}</div>
                  </div>
                  <div style={{ flex: 1, background: color.bg, borderRadius: 10, padding: "8px 10px" }}>
                    <div style={{ fontSize: 9, color: color.textMuted, textTransform: "uppercase", letterSpacing: 0.5 }}>{t(lang, "stats.worstMonth")}</div>
                    <div style={{ fontSize: 12, fontWeight: 700, color: color.negative, fontFamily: moneyFont, fontVariantNumeric: "tabular-nums", marginTop: 2 }}>{worst.label}</div>
                    <div style={{ fontSize: 10, color: color.negative }}>{worst.saldo >= 0 ? "+" : ""}{formattaValuta(worst.saldo)}</div>
                  </div>
                  <div style={{ flex: 1, background: color.bg, borderRadius: 10, padding: "8px 10px" }}>
                    <div style={{ fontSize: 9, color: color.textMuted, textTransform: "uppercase", letterSpacing: 0.5 }}>{t(lang, "stats.avgExpensesLabel")}</div>
                    <div style={{ fontSize: 12, fontWeight: 700, color: color.warn, fontFamily: moneyFont, fontVariantNumeric: "tabular-nums", marginTop: 2 }}>
                      {formattaValuta(avgUscite)}
                    </div>
                    <div style={{ fontSize: 10, color: color.textMuted }}>{t(lang, "stats.perMonth")}</div>
                  </div>
                </div>
              );
            })()}
